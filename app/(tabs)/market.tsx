...
export default function MarketPrices() {
  ...
  React.useEffect(() => {
    ...
    ;(async () => {
      try {
        // Load cached market data immediately
        const cached = await loadItem('last_market')
        if (cached && cached.data && mounted) {
          const list = Array.isArray(cached.data) ? cached.data : (cached.data.items || cached.data.prices || [])
          if (list && list.length) {
            setPrices(list.map((it: any) => ({
              name: it.name,
  ...
  React.useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        try {
          const res = await fetch(`${process.env.API_BASE || 'http://localhost:8000'}/api/market${q}`)
          if (res.ok) {
            ...
            if (list && list.length) {
              ...
              try {
                await saveItem('last_market', { data, timestamp: now })
              } catch {
                // ignore save errors
              }
            }
          ...
          // Keep showing cached data — no error shown
        }
      } finally {
  ...
  const { market: providerMarket, marketUpdatedAt: providerMarketUpdatedAt } = useAppData()
  React.useEffect(() => {
    if (providerMarket && Array.isArray(providerMarket) && providerMarket.length) {
      ...
      setLoading(false)
      if (providerMarketUpdatedAt) setLastUpdatedAt(providerMarketUpdatedAt)
    }
  }, [providerMarket, providerMarketUpdatedAt])
  ...
  const lastUpdatedStr = lastUpdatedAt
    ? `${t('updated_at')} ${new Date(lastUpdatedAt).toLocaleTimeString()} · ${new Date(lastUpdatedAt).toLocaleDateString()}`
    : null
  ...
  return (
    <ScrollView contentContainerStyle={{ padding: 16 }}>
      <View style={{ marginBottom: 12 }}>
        ...
        {lastUpdatedStr && (
          <Text style={styles.lastUpdated}>{lastUpdatedStr}</Text>
        )}
      </View>
...
const styles = StyleSheet.create({
  ...
  lastUpdated: { color: '#9CA3AF', marginTop: 4, fontSize: 12 },
