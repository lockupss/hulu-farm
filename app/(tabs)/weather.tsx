...
export default function Weather() {
        ...
        if (lat != null && lon != null) {
          try {
            ...
            try {
              ...
              } else if (mounted && !cached) {
                setError(t('weather_update_failed'))
              }
            } catch {
              if (mounted && !cached) setError(t('weather_update_failed'))
            }
          }
        } else {
          if (mounted && !cached) {
  ...
  const retryFetch = async (isPullToRefresh = false) => {
  ...
  {showingOfflineSnapshot && (
      ...
      {loading ? (
        ...
        <View style={{ padding: 16 }}>
          ...
          <View style={[styles.errorActionsRow, compact && styles.errorActionsRowCompact]}>
            <TouchableOpacity style={[styles.offlineBtn, compact && styles.compactButtonSpacing, { borderColor: colors.tint }]} onPress={retryFetch}>
              <Text style={[styles.offlineBtnText, { color: colors.tint }]}>{t('retry')}</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={[styles.offlineBtnFilled, compact && styles.compactButtonSpacing, { backgroundColor: colors.tint, marginLeft: compact ? 0 : 8 }]}
              onPress={async () => {
                try {
                  const off = await loadOfflineWeather()
                  if (off) {
            ...
            <View style={{ marginHorizontal: 16 }}>
              <View style={[styles.currentCard, { backgroundColor: colors.background, borderColor: '#E6F4EA' }]}>
                <View style={{ padding: 18, alignItems: 'center' }}>
                  ...
                  <Text style={styles.updatedText}>{t('updated_at')} {new Date(item.fetchedAt).toLocaleTimeString(dateLocale)}</Text>
                  ...
                  <View style={styles.metricsStack}>
                    ...
                    <View style={[styles.offlineRow, compact && styles.offlineRowCompact]}>
                    <TouchableOpacity
                      style={[styles.offlineBtn, compact && styles.compactButtonSpacing, { borderColor: colors.tint }]}
                      onPress={async () => {
                        try {
...
const styles = StyleSheet.create({
  ...
  errorActionsRowCompact: { flexDirection: 'column', gap: 8 },
  offlineRow: { flexDirection: 'row', marginTop: 12, width: '100%' },
  offlineRowCompact: { flexDirection: 'column', gap: 8 },
  ...
  compactButtonSpacing: { marginRight: 0 },
