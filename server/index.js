...
const { createSocialApi } = require('./social-store')
const socialApi = createSocialApi({ DATA_DIR, readJSON, writeJSON })

function serveRest(req, res, url) {
  ...
  if (req.method === 'POST' && pathname === '/api/forum') {
    ...
    req.on('end', () => {
      try {
        const uid = socialApi.getBearerUserId(req)
        if (!uid) {
          res.writeHead(401, { 'Content-Type': 'application/json' })
          res.end(JSON.stringify({ error: 'sign_in_required' }))
          return
        }
        const user = socialApi.findUserById(uid)
        const payload = JSON.parse(body)
        // normalize post shape for persistence
  const post = Object.assign({ replies: [], likes: 0, liked_by: [], category: 'General' }, payload)
  post.authorUserId = uid
  post.author = user ? user.displayName : 'Farmer'
  post.authorAvatar = user && user.avatarUrl ? user.avatarUrl : null
  // ensure time fields
  ...
  if (req.method === 'POST' && pathname.match(/^\/api\/forum\/[0-9]+\/reply$/)) {
    ...
    req.on('end', () => {
      try {
        ...
        const user = socialApi.findUserById(uid)
        const payload = JSON.parse(body)
        payload.authorUserId = uid
        payload.author = user ? user.displayName : 'Farmer'
        if (user && user.avatarUrl) payload.authorAvatar = user.avatarUrl
  ...
  const posts = normalizePosts(readJSON('forum_posts.json') || [])
        ...
        const sessionUid = socialApi.getBearerUserId(req)
        if (!sessionUid) {
          res.writeHead(401, { 'Content-Type': 'application/json' })
          res.end(JSON.stringify({ error: 'sign_in_required' }))
          return
...
const server = http.createServer((req, res) => {
  ...
  socialApi.handle(req, res, url).then((handled) => {
    if (handled) return
    serveRest(req, res, url)
  }).catch((e) => {
    console.error(e)
    if (!res.headersSent) {
