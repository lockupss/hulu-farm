...
/** When using Expo Go on a phone, Metro's host is your PC's LAN IP — reuse it for the Django port. */
...
async function probeUrl(base: string, path = '/api/v1/auth/me/', timeout = 3500) {
  try {
    ...
    clearTimeout(id)
    return res.status < 500 // 401 from Django means server is alive
  } catch {
    return false
  }
}
...
async function ensureBase() {
  ...
  // On web, try same origin first (if you proxy /api to Django).
  if (typeof window !== 'undefined' && window.location) {
    candidates.push('')
  }

  candidates.push('http://localhost:8000')
  candidates.push('http://10.0.2.2:8000') // Android emulator → host loopback
  ...
  cachedBase = configured || envPublicBase() || lan || 'http://localhost:8000'
  return cachedBase
}
...
async function parseErrorDetail(res: Response): Promise<string> {
  try {
    ...
    try {
      ...
      return String(j.detail || j.error || j.message || (j.non_field_errors?.[0]) || text)
    } catch {
...
/** Resolve `/media/...` paths from the Django server for Image URIs. */
